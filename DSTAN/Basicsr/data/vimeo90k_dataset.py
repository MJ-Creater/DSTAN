...
class Vimeo90KDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.neighbor_list = [
            i + (8 - opt['num_frame']) // 2 for i in range(opt['num_frame'])     # [1, 2, 3, 4, 5, 6, 7]
        ]
    ...
    def __getitem__(self, index):
        ...
        if self.opt['all_gt']:
            ...
            for neighbor in self.neighbor_list:
                if self.is_lmdb:
                    img_gt_path = f'{key}/{neighbor}'
                else:
                    # img_gt_path = self.gt_root / clip / seq / f'im{neighbor}.png'
                    img_gt_path = self.gt_root / clip / f'im{neighbor}.png'
            ...
            if self.is_lmdb:
                ...
                img_gt_path = self.gt_root / clip / 'im4.png'
            img_bytes = self.file_client.get(img_gt_path, 'gt')
            img_gt = imfrombytes(img_bytes, float32=True)
        ...
        for neighbor in self.neighbor_list:
            if self.is_lmdb:
                img_lq_path = f'{clip}/{neighbor}'
            else:
                # img_lq_path = self.lq_root / clip / seq / f'im{neighbor}.png'
                img_lq_path = self.lq_root / clip / f'im{neighbor}.png'
