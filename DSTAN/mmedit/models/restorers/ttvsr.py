...
from mmedit.utils.crop_validation import forward_crop
...
class TTVSR(BasicRestorer):
    ...
    def forward_test(self,
        ...
        with torch.no_grad():
            ...
            # 裁剪恢复crop_restoration
            if self.crop_restoration:
                crop_lq_size = 64
                overlap = crop_lq_size // 2   # TODO

                output = forward_crop(lq, self.generator, lq_size=crop_lq_size, overlap=overlap)
            else:
                output = self.generator(lq)
            ...
            if output.ndim == 4:  # an image, key = 000001/0000 (Vimeo-90K)
                ...
                for i in range(0, output.size(1)):
                    if isinstance(iteration, numbers.Number):
                        ...
                        # for REDS test
                        save_path_i = osp.join(save_path, folder_name,
                                               str(int(folder_name))+'_'+f'{i+1:03d}.png')
                    else:
                        raise ValueError('iteration should be number or None, '
                                         f'but got {type(iteration)}')
                    mmcv.imwrite(
