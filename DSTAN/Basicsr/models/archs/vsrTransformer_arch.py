...
class FeedForward(nn.Module):
    def __init__(self, num_feat):
        ...
        # self.backward_resblocks = ResidualBlocksWithInputConv(num_feat+3, num_feat, num_blocks=20)
        # self.forward_resblocks = ResidualBlocksWithInputConv(num_feat+3, num_feat, num_blocks=20)
        # self.fusion = nn.Conv2d(num_feat*2, num_feat, 1, 1, 0, bias=True)
        # self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=True)
        self.relu = nn.ReLU(inplace=True)
        self.fc1 = nn.Linear(num_feat, 2*num_feat)
        self.fc2 = nn.Linear(2*num_feat, num_feat)
        
    def forward(self, x, lrs=None, flows=None):
        b, t, c, h, w = x.shape
        out = self.fc2(self.relu(self.fc1(x.view(-1, c, h, w))))
...
class globalAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_x = attn_x.permute(0, 1, 3, 2).contiguous()  # [B, 5, H, 8*8*64/H, 8*8]
...
class vsrTransformer(nn.Module):
    ...
    def forward(self, x): 
        ...
        out = self.lrelu(self.conv_hr(feat))                              # [B, 64, 256, 256] -> [B*5, 64, 256, 256]
