...
class GenerateSegmentIndices:
    ...
    def __init__(self, interval_list, start_idx=0, filename_tmpl='{:03d}.png'):
        self.interval_list = interval_list
        self.filename_tmpl = filename_tmpl
    ...
    def __call__(self, results):
        ...
        gt_path_root = results['gt_path']
        lq_path = [
            osp.join(lq_path_root, clip_name, str(int(clip_name))+'_' + self.filename_tmpl.format(v+1))
            for v in neighbor_list
        ]
        gt_path = [
            osp.join(gt_path_root, clip_name, str(int(clip_name))+'_' + self.filename_tmpl.format(v+1))
            for v in neighbor_list
        ]
        results['lq_path'] = lq_path
        results['gt_path'] = gt_path
